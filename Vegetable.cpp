...
void Vegetable::splitSell(int splits,int  input1,int input2,int input3, int input4,string date,
  ...
  string d1,d2,d3,d4,c1,c2,c3,c4;
  d1=remainingArray[selection1].getDate();
  d2=remainingArray[selection2].getDate();
  d3=remainingArray[selection3].getDate();
  d4=remainingArray[selection4].getDate();
  c1=remainingArray[selection1].getCompany();
  c2=remainingArray[selection2].getCompany();
  c3=remainingArray[selection3].getCompany();
  c4=remainingArray[selection4].getCompany();
  ...
  int change[4];
  if(input1){
      selArray[0]= selection1;
      inArray[0] = input1;
      k++;
  }
  if(splits > 1 && input2){
      selArray[k] = selection2;
      inArray[k] = input2;
      k++;
  }
  if(splits > 2 && input3){
      inArray[k] = input3;
      selArray[k] = selection3;
      k++;
  }
  if(splits > 3 && input4){
      inArray[k] = input4;
      selArray[k] = selection4;
      k++;
  }
  for(int i = 0; i < k; i++){

      //update remaining, take into account if it has return or not
      if (remainingArray[selArray[i]].getReturn()){
          change[i]= min(inArray[i],remainingArray[selArray[i]].getReturn());
          //update the remaining array, along with the remaining num inside of it
          remainingArray[selArray[i]].updateRemainingWithRet((-1) * inArray[i]);
          //update the return element
      }
      else{

          //if not in remain array, just update the return element

          remainingArray[selArray[i]].updateRemaining((-1) * inArray[i]);
          change[i]= 0;
      }
      ...
      while(left){
          int temp = returnExistCompany( remainingArray[selArray[i]].getCompany(),
                                  remainingArray[selArray[i]].getDate());
    ...
    //needs to be last or else selection will get messed up
      if(remainingArray[selArray[i]].getRemaining() == 0){
          if(remainingNum != 1){
            for(int j = selArray[i]; j < remainingNum-1; j++){
              remainingArray[j]= remainingArray[1+j];
            }
            for( int z = i; z < k; z++){
              selArray[z]--;
            }
      ...
      newHist.splitSell(splits, input1, input2, input3,  input4, date, customer,
                        price, d1,d2,d3,d4,c1,c2,c3,c4,
